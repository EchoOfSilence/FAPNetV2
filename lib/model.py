...
class ASPP(nn.Module):
    def __init__(self, in_channels, out_channels):
        self.__init__ = super(ASPP, self).__init__()
    ...
    def forward(self, x):
        ...
        branch5_output = self.branch5(x)
        branch5_output = nn.Upsample(size=branch1_output.size()[2:], mode='bilinear', align_corners=False)(branch5_output)

        # print("Branch 1 output size:", branch1_output.size())
        # print("Branch 2 output size:", branch2_output.size())
        # print("Branch 3 output size:", branch3_output.size())
        # print("Branch 4 output size:", branch4_output.size())
        # print("Branch 5 output size:", branch5_output.size())
        # 将各个分支的输出在通道维度进行拼接
        concatenated = torch.cat((branch1_output, branch2_output, branch3_output, branch4_output, branch5_output),dim=1)

        # 利用1x1卷积操作进行降维，得到最终的特征表示
        output = self.reduce_channels(concatenated)
