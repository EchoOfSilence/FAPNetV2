...
class PolypObjDataset(data.Dataset):
    def __init__(self, image_root, gt_root, edge_root, trainsize):
        ...
        DATASET_ROOT = '/media/data2/zhangcheng/data'
        # 训练集和验证集
        image_root = os.path.join(DATASET_ROOT, 'TrainDataset/Imgs/')
        gt_root = os.path.join(DATASET_ROOT, 'TrainDataset/GT/')
        edge_root = os.path.join(DATASET_ROOT, 'TrainDataset/Edge/')

        self.trainsize = trainsize
        # # get filenames
        # self.images = [image_root + f for f in os.listdir(image_root) if f.endswith('.jpg')]
        # self.gts = [gt_root + f for f in os.listdir(image_root) if f.endswith('.jpg') or f.endswith('.png')]
